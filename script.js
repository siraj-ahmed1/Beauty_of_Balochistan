document.addEventListener('DOMContentLoaded', function () {
            ...
            on: {
                slideChangeTransitionStart: function () {
                    ...
                    if (content) {
                        ...
                        content.style.animation = 'slideIn 0.9s ease-out both';
                    }
                }
            }
        });
    ...
    // ── Slide-background zoom setup ───────────────────────────────
    document.querySelectorAll('.heroSwiper .swiper-slide').forEach(function (slide) {
    ...
    // ══════════════════════════════════════════════════════════════
    // ── Hero Video — Lazy Load + Mobile Optimisation ──────────────
    //
    // WHY THIS FIXES MOBILE SLOWNESS:
    // Without this block, the browser starts downloading the video
    // the instant the HTML is parsed — even on a slow 4G connection.
    // A large MP4 can block the rest of the page from feeling ready.
    //
    // What this block does:
    // 1. Detects slow mobile connections (2G / Data Saver mode)
    //    and skips the video entirely — poster image shows instead.
    // 2. Uses IntersectionObserver so the video only starts
    //    downloading when the hero section is actually on screen.
    // 3. Swaps data-src → src on <source> tags to trigger the load.
    // ══════════════════════════════════════════════════════════════
    (function () {
        var video = document.getElementById('heroVideo');
        if (!video) return;

        // ── Step 1: Skip video on slow/metered mobile connections ──
        // The Network Information API is supported on Chrome/Android.
        // On slow-2g, 2g, or when the user has enabled "Data Saver",
        // we hide the video and let the poster image do its job.
        ...
        if (isSlow) {
            // The poster attribute on the <video> already shows the
            // static image — removing the element keeps it visible.
            video.style.display = 'none';
            return;
        }

        // ── Step 2: Lazy-load the video when the hero is visible ───
        // Sources use data-src (not src) in the HTML, so no byte is
        // fetched until this function runs.
        function loadVideo() {
            video.querySelectorAll('source[data-src]').forEach(function (source) {
                source.src = source.dataset.src;
            });

            // Tell the browser to re-scan its <source> elements now
            // that they have real src values.
            video.load();

            // Attempt autoplay (works fine for muted videos on mobile)
            video.play().catch(function () {
                // Autoplay blocked in some browsers — poster is shown.
            });
        }

        // ── Step 3: Observe the hero, load only when visible ───────
        if ('IntersectionObserver' in window) {
            var heroObserver = new IntersectionObserver(function (entries) {
                if (entries[0].isIntersecting) {
                    loadVideo();
                    heroObserver.disconnect(); // only fire once
                }
            }, { threshold: 0.1 });

            heroObserver.observe(video);
        } else {
            // Older browsers that don't support IntersectionObserver
            // get the video loaded immediately as a safe fallback.
            loadVideo();
        }
    })();
...
/* ============================================================
   GALLERY — Lightbox  ·  Filter  ·  Load More  ·  Mute Toggle
   ============================================================ */
(function () {
...
/* ── Newsletter Subscribe → YouTube redirect ── */
function handleSubscribe(e) {
    e.preventDefault();
    window.open('https://www.youtube.com/@codding-zone', '_blank');
    e.target.reset();
}